...
Create commits that follow the repository standards and avoid mixing unrelated work.

## Workflow

1. Inspect the working tree:
   - Run `git status --short`.
   - Review relevant unstaged and staged diffs with `git diff` and `git diff --cached`.
   - Identify files modified by the current task versus unrelated existing changes.
...
4. Create atomic commits:
   ...
   - If a file contains mixed changes, stage only the task-related hunks when practical. If that is risky or unclear, ask the user how to proceed.
...
- Never use destructive commands such as `git reset --hard` or `git checkout --` unless the user clearly asks for that operation.
